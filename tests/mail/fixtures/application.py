...
async def run_examples(app: Application) -> dict[str, object]:
    ...
    require(
        json.loads(http_response.getBody())["status"] == "stored",
    ...
    await Mail.send(
        Content(text="plain", html="<p>html</p>"),
        callback=configure_welcome,
    )
    ...
    for path in files:
        parsed = BytesParser(policy=policy.default).parsebytes(path.read_bytes())
        require(
            parsed["Bcc"] is None and parsed["Sender"] is None,
            "The MIME headers leak hidden recipients or invent Sender.",
        )
        require(
            parsed["Date"] is not None and parsed["From"] is not None,
    ...
    require(
        not list(app.basePath.glob("outgoing/**/*.tmp")),
        "Temporary files remain.",
    )
    return {
        "files": len(files),
        "extension": result.driver,
...
async def main(runtime: str) -> None:
    ...
    app.config(
        "view",
        {"paths": [str(Path(__file__).parent)], "cache_path": None, "autoescape": True},
    )
    app.config(
        "filesystems",
        {
