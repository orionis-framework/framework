...
class _DefaultResponses:
    ...
    @staticmethod
    def error(*, status_code: int, content: str, **_kwargs: object) -> Response:
    ...
    def testMatchesExactAndWildcardHosts(self) -> None:
        ...
        for hostname in (
            ...
            with self.subTest(hostname=hostname):
                self.assertIsNone(middleware.handle(make_adapter([
                    (b"host", hostname),
                ])))
        for hostname in (b"evil-example.com", b"example.com.evil", b"unknown"):
            with self.subTest(hostname=hostname):
                response = middleware.handle(make_adapter([(b"host", hostname)]))
                self.assertEqual(response.getStatusCode(), 400)

    def testRetainsHeaderInjectionValidation(self) -> None:
        """Reject carriage returns and line feeds before accepting the request."""
        middleware = SecurityMiddleware({}, _DefaultResponses())
        for headers in (
            [(b"x-value", b"text\r\ninvalid")], [(b"x-invalid\n", b"text")],
        ):
            response = middleware.handle(make_adapter(headers))
            self.assertEqual(response.getStatusCode(), 400)
